...
for current_tissue in tissues:
    ...
    # Set current classifier to linear SVM:
    current_algorithm="linearSVM"

    # Train and fit without a standard scaler since it scaling is not recommended for the sparse single-cell data:
    HCL_trained_pipe = make_pipeline(LinearSVC(random_state=1))
    ...
    MCA_trained_pipe = make_pipeline(LinearSVC(random_state=1))
    ...
    # Pergorm transfer learning with TrAdaBoost (instance based transfer learning)
    ...
    MCA_trained_TrAdaBoost = TrAdaBoost(LinearSVC(random_state=1), n_estimators=10, Xt=HCL_X_train[:targetAvailable], yt=HCL_y_train[:targetAvailable], random_state=1)
